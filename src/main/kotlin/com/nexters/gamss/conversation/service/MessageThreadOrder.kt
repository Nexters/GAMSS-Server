 ...
 * 옮겨, 클라이언트가 받는 목록이 실제 대화 스레드처럼 읽히게 재배열한다. 유저 메시지나 유저 답글에
 * 캐릭터가 다시 응답한 메시지처럼 캐릭터 댓글이 아닌 대상에게 답장하는 메시지는 건드리지 않는다 —
 * 이미 시간순으로 답장 대상 바로 다음에 저장되기 때문이다.
 *
...
internal object MessageThreadOrder {
    fun reorderTikitakaAfterTarget(messages: List<Message>): List<Message> {
        ...
        val tikitakaByTarget =
            messages
                .filter { it.repliesToMessageId != null && messageById[it.repliesToMessageId]?.senderType == SenderType.CHARACTER }
