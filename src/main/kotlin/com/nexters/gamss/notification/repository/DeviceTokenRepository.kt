...
interface DeviceTokenRepository : JpaRepository<DeviceToken, Long> {
     ...
     *
     * 소유자를 **옮기는** 것이 맞는 동작이다. FCM 토큰은 계정이 아니라 기기(앱 설치)에 붙으므로,
     * 같은 기기에서 A 가 로그아웃하고 B 가 로그인하면 **같은 토큰**이 B 의 것으로 다시 등록된다.
     *
     * - 거절하면 B 는 알림을 못 받는다. 그 기기의 토큰은 여전히 A 것으로 남아 있다.
     * - 새 행을 만들면 `(A, 토큰)` 과 `(B, 토큰)` 이 함께 남는다. 그러면 **A 앞으로 나간 알림이 지금
     *   그 기기를 쓰는 B 에게 뜬다** — 알림 문구에 카드 한 줄이 들어가므로 남의 감정 기록이 잠금
     *   화면에 노출된다.
     *
     * 앱이 로그아웃 때 해제 API 를 부르면 이 상황 자체가 생기지 않지만, 그 호출은 오프라인·크래시·
     * 구버전 앱에서 빠질 수 있다. 서버가 제약으로 못 박아야 하는 이유다.
     *
