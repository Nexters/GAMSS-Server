    ...
    @Operation(
        ...
        description =
            "공유 링크(`gamss.kr/c/{shareToken}`)를 연 사람에게 보여줄 카드입니다. " +
                ...
                "**이 API 를 쓰는 것은 웹 페이지뿐입니다.** 링크를 누른 사람에게 앱이 있으면 OS 가 " +
                "앱으로 넘기고, 앱은 토큰을 쓰지 않고 메인 화면을 엽니다(기획 결정). 앱이 없을 때만 " +
                "같은 주소가 웹 페이지로 열리고, 그 페이지가 이 응답으로 카드를 그립니다.\n\n" +
                "앱이 나중에 공유된 카드 화면을 붙이면 그대로 쓰면 됩니다 — 로그인 상태로 불러도 " +
                "응답은 같습니다.\n\n" +
