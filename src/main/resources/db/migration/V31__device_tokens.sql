...
--
-- token 유니크가 필요한 이유: FCM 토큰은 계정이 아니라 기기에 붙어서, 같은 기기에서 A 가 로그아웃하고
-- B 가 로그인하면 **같은 토큰**이 다른 회원으로 다시 등록된다. 유니크가 없으면 (A, 토큰)·(B, 토큰)
-- 두 행이 남고, A 앞으로 나간 알림이 지금 그 기기를 쓰는 B 에게 뜬다 — 알림 문구에 카드 한 줄이
-- 들어가므로 남의 감정 기록이 잠금화면에 노출된다.
