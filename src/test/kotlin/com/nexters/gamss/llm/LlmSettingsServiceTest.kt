...
class LlmSettingsServiceTest {
    ...
    fun `DB에 값이 없으면 코드 기본값을 반환한다`() {
        ...
        assertEquals(promptProvider.systemPrompt, current.systemPrompt)
    }

    @Test
    fun `DB에 값이 있으면 그 값을 반환한다`() {
        every { repository.findAll() } returns listOf(LlmSettings("gemini-2.5-flash", "커스텀 프롬프트"))
    ...
    fun `설정이 없으면 새로 저장한다`() {
        ...
        service.update("gemini-2.5-flash", "새 프롬프트")
    ...
    fun `설정이 있으면 기존 행을 갱신한다`() {
        val row = LlmSettings("gemini-3.1-flash-lite", "old")
        every { modelCatalog.availableModels() } returns listOf("gemini-2.5-flash")
        every { repository.findAll() } returns listOf(row)

        service.update("gemini-2.5-flash", "new")
    ...
    fun `gemini 접두사가 아니면 INVALID_INPUT`() {
        val exception = assertFailsWith<BusinessException> { service.update("gpt-4", "p") }
    ...
    fun `카탈로그에 없는 모델이면 INVALID_INPUT`() {
        ...
        val exception = assertFailsWith<BusinessException> { service.update("gemini-9.9-ultra", "p") }
