 ...
 *
 * 탈퇴는 회원 행을 소프트 삭제하고 `social_accounts` 를 하드 삭제하므로, 같은 소셜 계정으로 다시
 * 로그인하면 **새 memberId** 를 받는다. 그 격리는 의도된 설계다
 * ([com.nexters.gamss.auth.service.WithdrawnMemberRejoinIntegrationTest]). 문제는 그 때문에 탈퇴 이전
 * 사용량과 재가입 후 회원을 이어줄 식별자가 없어, 한도를 다 쓴 뒤 탈퇴하고 재가입하면 한도가
 * 리셋되던 것이다.
 *
 * 커밋된 상태를 봐야 하므로 클래스 레벨 트랜잭션 롤백을 쓰지 않고 직접 정리한다
 * ([com.nexters.gamss.auth.service.WithdrawnMemberRejoinIntegrationTest] 와 같은 이유).
 *
...
class RejoinTokenLimitIntegrationTest {
    ...
    /** 이 이슈의 본론. 탈퇴와 재가입을 거쳐도 그 구간의 사용량이 그대로 남아야 한다. */
    @Test
    ...
    /** 재가입 후 쓴 것은 이전 사용량 위에 쌓인다. 같은 주체이므로 같은 행에 누적된다. */
    @Test
    ...
    /**
     * 사용량만 이어받고 대화·카드 격리는 그대로여야 한다. 회원 행과 소셜 계정이 새로 생기는 계약을
     * 여기서도 함께 확인한다 - 사용량을 잇기 위해 격리를 깨뜨리지 않았다는 증거다.
     */
    @Test
    ...
    /**
     * 탈퇴하면 회원과 주체의 연결이 끊긴다. 그래도 위 테스트들이 통과하는 것이 핵심이다 - 이월을
     * 떠받치는 것은 이 매핑이 아니라 주체 키의 결정론성이고, 재가입이 같은 키로 다시 잇는다.
     */
    @Test
    fun `탈퇴하면 주체 매핑이 끊긴다`() {
        authService.login("idtok")
