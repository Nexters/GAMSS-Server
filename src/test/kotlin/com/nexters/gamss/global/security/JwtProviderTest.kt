...
class JwtProviderTest {
    ...
    fun `액세스 토큰을 발급하고 memberId를 파싱한다`() {
        val token = jwtProvider.issueAccessToken(42L)

        assertEquals(42L, jwtProvider.parseMemberId(token))
    }

    @Test
    fun `리프레시 토큰도 memberId를 파싱한다`() {
        val token = jwtProvider.issueRefreshToken(7L)

        assertEquals(7L, jwtProvider.parseMemberId(token))
    }

    @Test
    fun `서명이 다른 토큰은 INVALID_TOKEN 예외를 던진다`() {
        val other = JwtProvider(properties.copy(secret = "another-secret-value-also-long-enough-for-hs256-abcdefghijklmnop"))
        val forged = other.issueAccessToken(1L)

        val exception = assertFailsWith<BusinessException> { jwtProvider.parseMemberId(forged) }
    ...
    fun `만료된 토큰은 EXPIRED_TOKEN 예외를 던진다`() {
        val shortLived = JwtProvider(properties.copy(accessTokenValidity = Duration.ofMillis(1)))
    ...
    fun `형식이 잘못된 토큰은 INVALID_TOKEN 예외를 던진다`() {
        val exception = assertFailsWith<BusinessException> { jwtProvider.parseMemberId("not-a-valid-jwt") }
