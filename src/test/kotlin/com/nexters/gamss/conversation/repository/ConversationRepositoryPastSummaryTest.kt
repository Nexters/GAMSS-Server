...
class ConversationRepositoryPastSummaryTest {
    ...
    fun `과거 요약은 같은 회원의 최근 대화방 중 요약 있는 것만, 최대 pickCount개 무작위로 반환한다`() {
        ...
        repeat(3) {
            val saved = conversationRepository.save(Conversation(memberId))
            conversationRepository.updateSummary(saved.id, "요약-$it")
        }
        conversationRepository.save(Conversation(memberId)) // summary 없는 대화방 — 후보에서 제외돼야 함
        val otherMember = conversationRepository.save(Conversation(memberId = 2L))
        conversationRepository.updateSummary(otherMember.id, "다른 회원 요약")
    ...
    fun `삭제된 대화방의 요약은 후보에서 제외된다`() {
        ...
        val active = conversationRepository.save(Conversation(memberId))
        conversationRepository.updateSummary(active.id, "살아있는 방 요약")

        val picked = conversationRepository.findRandomPastSummaries(memberId, current.id, poolSize = 5, pickCount = 5)

        assertEquals(listOf("살아있는 방 요약"), picked)
    }
    ...
    fun `과거 요약 풀이 pickCount보다 적으면 있는 만큼만 반환한다`() {
        val memberId = 1L
        val current = conversationRepository.save(Conversation(memberId))
        val onlyOne = conversationRepository.save(Conversation(memberId))
        conversationRepository.updateSummary(onlyOne.id, "유일한 요약")
